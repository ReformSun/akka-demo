...
import com.test.rpc.akka.exceptions.AkkaHandshakeException;
import com.test.rpc.akka.exceptions.AkkaRpcException;
import com.test.rpc.akka.exceptions.AkkaUnknownMessageException;
import com.test.rpc.akka.messages.Processing;
import com.test.rpc.exceptions.RpcConnectionException;
import com.test.rpc.inter.MainThreadValidatorUtil;
...
public class TestAkkaRpcActor<T extends RpcGateway> extends UntypedActor {
    ...
    private  MainThreadValidatorUtil mainThreadValidator;

    private final CompletableFuture<Boolean> terminationFuture;

    private final int version;

    private TestAkkaRpcActor.State state;

    public TestAkkaRpcActor(final T rpcEndpoint, final CompletableFuture<Boolean> terminationFuture, final int version) {
        this.rpcEndpoint = checkNotNull(rpcEndpoint, "rpc endpoint");
//        this.mainThreadValidator = new MainThreadValidatorUtil(rpcEndpoint);
        this.terminationFuture = checkNotNull(terminationFuture);
        this.version = version;
        this.state = TestAkkaRpcActor.State.STOPPED;
    }
...
//        }
     ...
     *
     * 接受的客户端中的消息种类有很多中
     * 一种：远程握手的消息 为了是验证两个终端是否是兼容的
     * 验证方式：一验证两个终端的版本是否相等
     * 验证方式：二验证客户终端传过来的rpc终端类类型必须是本地rpcEndpoint类的类型的子类或者接口类
     * 二种：Processing 控制服务端AkkaRpcActor行为的状态值
     * 分为两种状态：开始和停止
     * 三种：rpc调用消息
     * 进入handleRpcMessage方法查看
     *
     *
     * @param message
     */
    ...
    public void onReceive(final Object message) {
        if (message instanceof RemoteHandshakeMessage) {
            ...
            state = TestAkkaRpcActor.State.STARTED;
        } else if (message.equals(Processing.STOP)) {
            state = TestAkkaRpcActor.State.STOPPED;
        } else if (state == TestAkkaRpcActor.State.STARTED) {
//            mainThreadValidator.enterMainThread();
...
//                mainThreadValidator.exitMainThread();
    ...
    /**
     * 一种：RunAsync 异步执行Runnable 无返回值
     * 二种：CallAsync 异步执行Callable 有返回值
     * 三种：RpcInvocation 具体调用某个类的包装类 通过它可以知道调用服务端的类的方法信息
     *
     */
    ...
    /**
     * 传过来的rpc终端类是否是rpcEndpoint类的子类或者接口
     * 只有满足这个要求才是被支持的rpc终端
     * @param rpcGateway
     * @return
     */
